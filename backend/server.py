...
async def store_get_session(session_token: str) -> Optional[Dict[str, Any]]:
  ...
  return MEM_SESSIONS.get(session_token)
...
async def analyze_food(request_data: AnalyzeFoodRequest, current_user: Optional[User] = Depends(get_current_user)):
  ...
  if not EMERGENT_LLM_KEY or LlmChat is None:
    raise HTTPException(status_code=503, detail="LLM not configured (set EMERGENT_LLM_KEY)")

  # If you later implement real LlmChat, this endpoint will work.
  raise HTTPException(status_code=501, detail="LLM shim active. Replace emergentintegrations shim with real implementation.")
